        ...
        i_checklistItems.forEach(item => {
            if (category.category == item.itemCategory) {
                ...
                eleCategory.append('' +
                        ...
                        '<p><input type="checkbox" id="checkbox_' + itemID + '" onclick="checkboxChange(this.id,this.checked,updatechecklistCategories)"' + ((isChecked) ? 'checked' : '') + '/> ' + itemValue + '</p>' +
                        '</div>' +
                        ''
                    );
            }
...
function updatechecklistCategories() {
    ...
    updateHTMLcategories(g_checklistCategories)

}

function updateHTMLcategories(i_checklistCategories) {
    // console.log('i have been passed through:' + JSON.stringify(i_checklistCategories))
