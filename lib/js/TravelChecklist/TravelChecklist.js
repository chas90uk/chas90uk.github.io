...
function setHTMLcategories(i_checklistCategories) {
    ...
    for (i in i_checklistCategories) {
        ...
        eleChecklistItemList.append('' +
            '<div id="collapsible_' + itemCategory + '" class="collapsible w3-show w3-animate-opacity w3-left-align w3-padding-small" onclick=collapsible(this.id)>' +
                '<p>' +
...
function collapsible(collapsibleID) {
    ...
    var eleCollapsible = $(collapsible)
    if (eleCollapsible.style.display === "block") {
        eleCollapsible.style.display = "none";
    }
    else {
        eleCollapsible.style.display = "block";
    }
}
////
